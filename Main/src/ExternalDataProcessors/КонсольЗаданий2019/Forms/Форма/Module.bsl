...
Процедура ВывестиФоновые()
	...
	// при отборе по регламентному заданию следует учесть вручную запущенные задания "Запуск вручную: Автоматическая привязка платежей консигнация"
	...
	Если ТипЗнч(Отбор) = Тип("Структура") И Отбор.Свойство("РегламентноеЗадание", РегламентноеЗаданиеОтбора) Тогда
		Отбор.Удалить("РегламентноеЗадание");
		Отбор.Вставить("Наименование", "Запуск вручную: " + РегламентноеЗаданиеОтбора.Метаданные.Синоним);
		Попытка
			ФоновыеРучные = ФоновыеЗадания.ПолучитьФоновыеЗадания(Отбор);
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			СообщитьПользователю(ТекстОшибки);
			Возврат;
	...
	СписокФоновыхЗаданий.Сортировать("Начало");
