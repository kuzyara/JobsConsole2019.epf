...
Процедура ОбновитьСписокРегламентныхЗаданий()
	...
	Для Каждого Регламентное из Регламентные Цикл
		...
		ОтсортированныеСтроки = СписокФоновыхЗаданий.Выгрузить(Строки);
		ОтсортированныеСтроки.Сортировать("Начало Убыв");
		Если Строки.Количество() > 0 Тогда
			НоваяСтрока.Выполнялось = ОтсортированныеСтроки[0].Начало;
			НоваяСтрока.Состояние = ОтсортированныеСтроки[0].Состояние;
			ИмяРегламентногоЗадания = НоваяСтрока.Метаданные + ":" + НоваяСтрока.Наименование;
