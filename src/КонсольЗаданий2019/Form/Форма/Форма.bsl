...
Процедура СписокФоновыхЗаданийПриАктивизацииСтрокиНаСервере(ТекущаяСтрокаРегламентное)
	Фоновое = ФоновыеЗадания.НайтиПоУникальномуИдентификатору(ТекущаяСтрокаРегламентное);
	Если Фоновое <> Неопределено Тогда
		...
		Если Фоновое.РегламентноеЗадание <> Неопределено Тогда
			...
			ТекущаяСтрокаРегламентное = Строка;
		Иначе
			ТекущаяСтрокаРегламентное = "";
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		Для ИденификаторТекСтроки = ТекущаяСтрока По ТекущаяСтрока Цикл
			...
			Если ТекСтрока <> Неопределено И ТипЗнч(ТекСтрока.Регламентное) = Тип("УникальныйИдентификатор") Тогда
				СписокФоновыхЗаданийПриАктивизацииСтрокиНаСервере(ТекСтрока.Регламентное);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
